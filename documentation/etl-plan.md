...
- [ ] Implement exact `memberCode` to Wikidata P4690 lookup.
- [ ] Reject multiple external entities claiming the same Oireachtas identifier pending review.
- [ ] Record unmatched Members without inventing a fuzzy match.
...
- [ ] Record the Wikidata Q-ID as the primary external identity anchor for matched Members.
...
Use separate external-link graphs, for example:

```text
https://data.oireachtas.ie/graph/external/wikidata
https://data.oireachtas.ie/graph/external/dbpedia
```

- [ ] Publish only accepted identity links.
- [ ] Ensure external-link graphs can be rebuilt without regenerating core Oireachtas graphs.
- [ ] Add graph-boundary tests proving that enrichment does not alter endpoint-owned authoritative graphs.
